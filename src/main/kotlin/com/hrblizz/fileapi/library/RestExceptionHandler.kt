...
class RestExceptionHandler(
    ...
    override fun handleExceptionInternal(
        ...
        val apiError = com.hrblizz.fileapi.rest.ResponseEntity<Any>(
            null, listOf(ErrorMessage(status.reasonPhrase)), status.value()
        )
    ...
    override fun handleMethodArgumentNotValid(
        ...
        this.log.error(ExceptionLogItem("Bad Request Exception: $errors", ex))
    ...
    override fun handleMissingServletRequestPart(
        ...
        val errorStatus = HttpStatus.BAD_REQUEST
        val apiError = com.hrblizz.fileapi.rest.ResponseEntity<Any>(
            null, listOf(ErrorMessage("${ex.requestPartName} part is missing")), errorStatus.value()
        )
        return handleExceptionInternal(ex, apiError, headers, errorStatus, request)
    }
    ...
    override fun handleMissingServletRequestParameter(
        ...
        val errorStatus = HttpStatus.BAD_REQUEST
        val apiError = com.hrblizz.fileapi.rest.ResponseEntity<Any>(
            null, listOf(ErrorMessage("${ex.parameterName} parameter is missing")), errorStatus.value()
        )
        return handleExceptionInternal(ex, apiError, headers, errorStatus, request)
    }
    ...
    fun handleMethodArgumentTypeMismatch(
        ...
        val errorStatus = HttpStatus.BAD_REQUEST
        val apiError = com.hrblizz.fileapi.rest.ResponseEntity<Any>(
            null, listOf(ErrorMessage("${ex.name} should be of type ${ex.requiredType?.name}")), errorStatus.value()
        )
    ...
    override fun handleNoHandlerFoundException(
        ...
        val apiError = com.hrblizz.fileapi.rest.ResponseEntity<Any>(
            null, listOf(ErrorMessage(errorStatus.reasonPhrase)), errorStatus.value()
        )
        return ResponseEntity(apiError, HttpHeaders(), errorStatus)
    }
    ...
    fun handleBadRequest(ex: BadRequestException): ResponseEntity<Any> {
        this.log.error(ExceptionLogItem("Bad Request exception: ${ex.message}", ex))
        var status: HttpStatus? = getResponseStatus(ex.javaClass)
        if (status == null) {
            status = HttpStatus.BAD_REQUEST
        }
        val apiError = com.hrblizz.fileapi.rest.ResponseEntity<Any>(
            null, listOf(ErrorMessage(ex.message)), status.value()
        )
    ...
    fun handleAccessDenied(ex: AccessDeniedException): ResponseEntity<Any> {
        this.log.error(ExceptionLogItem("Access Denied exception: ${ex.message}", ex))
        val errorStatus = HttpStatus.UNAUTHORIZED
        val apiError = com.hrblizz.fileapi.rest.ResponseEntity<Any>(
            null, listOf(ErrorMessage(ex.message)), errorStatus.value()
        )
        return ResponseEntity(apiError, HttpHeaders(), errorStatus)
    }
    ...
    fun handleAll(ex: Exception, request: WebRequest): ResponseEntity<Any> {
        ...
        val apiError = com.hrblizz.fileapi.rest.ResponseEntity<Any>(
            null, listOf(ErrorMessage("Unknown error occurred")), errorStatus.value()
        )
        return ResponseEntity(apiError, HttpHeaders(), errorStatus)
    }

    @ExceptionHandler(MultipartException::class)
    fun handleMultipartError(ex: MultipartException, request: WebRequest): ResponseEntity<Any> {
        this.log.error(ExceptionLogItem("Multipart exception: ${ex.localizedMessage}", ex))
        val errorStatus = HttpStatus.BAD_REQUEST
        val apiError = com.hrblizz.fileapi.rest.ResponseEntity<Any>(
            null, listOf(ErrorMessage(ex.message)), errorStatus.value()
        )
    ...
    fun handleApplicationException(ex: ApplicationException): ResponseEntity<Any> {
        this.log.error(ExceptionLogItem("Internal application exception: ${ex.message}", ex))
        var status: HttpStatus? = getResponseStatus(ex.javaClass)
        if (status == null) {
    ...
    fun handleFileNotFoundException(ex: NotFoundException): ResponseEntity<Any> {
        this.log.error(ExceptionLogItem("NotFound exception: ${ex.message}", ex))
        var status: HttpStatus? = getResponseStatus(ex.javaClass)
        if (status == null) {
